...
int main()
{
	 ...
	 *
	 * 需要借助 code/qemu/vfio.sh 来配置
	 *
	 * echo 0000:c8:00.0 | sudo tee /sys/bus/pci/drivers/hct/bind
	 *
	 * 假设设备 group 是 noiommu-25
	 *
	 * 我当时是在虚拟机中测试的吗? 仅仅 enable_unsafe_noiommu_mode 是没用的
	 *
	 * 但是从 vfio_ioctl_set_iommu 看，的确是动态可以配置的
	 *
	 */
	const char *group_path = "/dev/vfio/noiommu-0";
	const char *device_name = "0000:c8:00.0";
